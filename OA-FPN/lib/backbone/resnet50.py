...
class Bottleneck(nn.Module):
    def __init__(self, in_cha, neck_cha, out_cha, stride, has_bias=False):
        ...
        # self.bn2 = FrozenBatchNorm2d(neck_cha)  改动的地方1
        self.bn2 = nn.GroupNorm(num_groups=64, num_channels=neck_cha)
...
class ResNet50(nn.Module):
    def __init__(self, freeze_at, has_bias=False):
        ...
        self.bn1 = nn.GroupNorm(num_groups=64, num_channels=64)
    ...
    def _make_layer(self, num_blocks, in_channels, bottleneck_channels, out_channels, stride):
        layers = []
        for _ in range(num_blocks):  #每个block进行重复
