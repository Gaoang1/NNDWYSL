...
class FPN(nn.Module):
    ...
    def __init__(self, bottom_up, layers_begin, layers_end):
        ...
        in_channels = [256, 512, 1024, 2048]   #特征图每一层的输出维度
        fpn_dim = 256
        ...
        self.gn = nn.GroupNorm(num_groups=64,num_channels=fpn_dim)    # 加了GN和relu
    ...
    def forward(self, x):
        ...
        results = []
        prev_features = F.relu_(self.gn(self.lateral_convs[0](bottom_up_features[0])))
        results.append(F.relu_(self.gn(self.output_convs[0](prev_features))))
        for l_id, (features, lateral_conv, output_conv) in enumerate(zip(
            ...
            lateral_features = F.relu_(self.gn(lateral_conv(features)))
            prev_features = lateral_features + top_down_features
            results.append(F.relu_(self.gn(output_conv(prev_features))))
        if(self.output_e == 6):
