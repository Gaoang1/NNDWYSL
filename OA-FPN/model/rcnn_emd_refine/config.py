...
class Crowd_human:
    ...
    root_folder = 'D:/Project/datasets/CrowdHuman'
    image_folder = 'D:/Project/datasets/CrowdHuman/Images'
    train_source = os.path.join('D:/Project/datasets/CrowdHuman/annotation_train.odgt')
    eval_source = os.path.join('D:/Project/datasets/CrowdHuman/annotation_val.odgt')

class Config:
    output_dir = 'outputs'
    model_dir = os.path.join(output_dir, 'model_dump')
    eval_dir = os.path.join(output_dir, 'eval_dump')
    init_weights = 'D:/Project/CrowdDet-master/weights/mixUp0.2_labelSmooth0.1_resnet50_XBNBlock_GN_NG64_sgd_lr0.1_cos_wd0.0001.pth'
    ...
    momentum = 0.9
    weight_decay = 1e-4
    base_lr = 0.01 * 5.21  # 1e-3 * 1.25

    warm_iter = 1500
    max_epoch = 45  # 30
    lr_decay = [24, 43]  # [24,27]
    nr_images_epoch = 15000
    log_dump_interval = 200
