    ...
    for (images, gt_boxes, im_info), step in zip(data_iter, range(0, train_config.iter_per_epoch)):
        ...
        if epoch == 1 and step < train_config.warm_iter:
            ...
            lr_new = 0.33 * train_config.learning_rate + \
                    0.67 * alpha * train_config.learning_rate           # warmup,lr会从小慢慢增加到指定lr,alpha从0逐渐增加到1
        ...
        if rank == 0:
            if step % train_config.log_dump_interval == 0:
                ...
                # writer.add_scalar('loss', stastic_total_loss, step)   # dasd sadsad as sa sa sa
...
def train_worker(rank, train_config, network, config):
    ...
    # net.resnet50.load_state_dict(backbone_dict['state_dict'],strict=False)   #默认是strict = True

    net.resnet50.load_state_dict(backbone_dict, strict=False)

    #missing_keys, unexpected_keys = net.resnet50.load_state_dict(backbone_dict['state_dict'], strict=False)
